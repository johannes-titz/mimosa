...
The generated `docs/` directory can be deployed to static hosting such as
GitHub Pages. Package availability depends on precompiled WebAssembly binaries
from the webR package repository.
...
Mimosa automatically fits a binomial generalized linear mixed model when the selected dependent variable has exactly two observed values; other numeric responses use a Gaussian linear mixed model. For a dichotomous response, the second factor level (or the second value in sorted order) is modeled as the event.
